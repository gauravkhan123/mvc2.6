...
class Backend_books extends Secure_Controller {
...

	{
		...
		$this->controller 		= $this->config->item('backend').'/books';

		$this->alias			= "";

		$this->view_folder		= "backend/books/";		
...

	{
		...
		$data['coloums'] = $this->basic_model->list_result(array('id','name','sku','books_category_id','publish'),$this->table);		
...

	{
			...
			$data['title'] = "Add " . $this->title;
				...
				$data['categories'] = $this->basic_model->dropdown_list("SELECT id,name FROM books_categories WHERE 1 AND publish = '1' ORDER BY name ASC");	
