...
class Backend_Invoices extends Secure_Controller {
	...
	function __construct()
	{
		...
		$this->db_id 			= "Id";
		$this->controller 		= $this->config->item('backend').'/invoices';
		$this->alias			= "";
		$this->view_folder		= "backend/invoices/";
...
//							pr($searchVars);
							...
							foreach($searchVars as $value)
							{
								...
								if(isset($value[1]) && $value[1] != '')
								{
									...
									if($value[0] == 'manuscript_id')
									{
										$condition .= " AND m.Id = '".$value[1]."' ";	
									}	
									elseif($value[0] == 'discount')
									{
										$condition .= " AND o.discount  like '%".urlsafe($value[1])."%'";
									}	
									elseif($value[0] == 'payment')
									{
										$condition .= " AND o.payment  like '%".urlsafe($value[1])."%'";
									}	
									elseif($value[0] == 'title')
									{
										$condition .= " AND m.title  like '%".urlsafe($value[1])."%'";
									}																																			
									...
									{
										$condition .= " AND LEFT(o.date,10)  <= '".str_replace("~","-",$value[1])."'";
									}	
					...
					{
							$searchVars = "filter-manuscript_id:-manuscript_no:-title:-journal_id:-author_name:-author_email:-author_country:-discount:-payment:-todate:-fromdate:";	
					}
...
//					pr($condition);
	//				pr($searchVars);					
					...
					$config['base_url'] = site_url($this->config->item('backend').'/invoices/listing/'.$searchVars);
					
					
					$config['total_rows'] = $this->invoices_model->total_results_backend($condition);
					$config['per_page'] = 20;
					...
					$data['coloums'] = $this->invoices_model->listing_backend($condition,$offset,$config['per_page']);		
	...
	public function printinvoice()
	{
		...
		$data['orderdata'] = $this->order_model->orderdata($this->uri->segment(4));
		$data['manuscriptdata'] = $this->order_model->manuscriptdata($this->uri->segment(4));
		$data['authordata'] = $this->order_model->authordata($data['manuscriptdata']['author_id']);		
		$this->load->view($this->view_folder.'printinvoice',$data);						
	...
	public function editinvoice()
	{
		...
		$data['invoiccedetails'] = $this->basic_model->dropdown_list("SELECT * FROM `edited_invoices` where manuscript_id = '".$this->uri->segment(4)."'");	
		
		
		if(!empty($data['invoiccedetails']))
		{
				$data['invoiccedetails'] = $data['invoiccedetails'][0];
				
			if($this->input->post('submit')!='')
			{
				
					$this->form_validation->set_rules('Invoice Date', 'invoice_date', 'trim');
					$this->form_validation->set_rules('Invoice No', 'invoice_no', 'trim');
					$this->form_validation->set_rules('Login Id', 'login_id', 'trim|valid_email');
					$this->form_validation->set_rules('Billed To', 'billed_to', 'trim');
					$this->form_validation->set_rules('Manuscript No', 'manuscript_no', 'trim');
					$this->form_validation->set_rules('Manuscript Name', 'manuscript_name', 'trim');
					$this->form_validation->set_rules('Quantity', 'quantity', 'trim');
					$this->form_validation->set_rules('Unit Price', 'unitprice', 'trim');
					$this->form_validation->set_rules('Discount', 'discount', 'trim');
					$this->form_validation->set_rules('Total', 'total', 'trim');					
					$this->form_validation->set_rules('Total Amount', 'total_amount', 'trim');
					$this->form_validation->set_rules('Grand Total Amount', 'grand_total_amount', 'trim');					
					
					if ($this->form_validation->run() == FALSE)
					{
						$this->load->view($this->config->item('backend').'/invoices/editinvoice/editinvoice',$data);				
					}
					else
					{
						...
						unset($_POST['submit']);						
							...
							$_POST['updated_on'] = current_date("Y-m-d h:i:s");
							
							//pr($_POST);

							if($this->basic_model->update_a_record('edited_invoices', $_POST,'invoice_no',$_POST['invoice_no']))
							{
								$this->session->set_flashdata('success', 'Record added successfully.');
								redirect($this->controller.'/editinvoice/'.$_POST['manuscript_id']);
							}
							else
							{
			...
			if($this->input->post('submit')!='')
			{
					...
					else
					{
						...
						unset($_POST['submit']);						
							...
							$_POST['created_on'] = current_date("Y-m-d h:i:s");

							if($this->basic_model->add_a_record('edited_invoices', $_POST))
							{
								$this->session->set_flashdata('success', 'Record added successfully.');
								redirect($this->controller.'/editinvoice/'.$_POST['manuscript_no']);
							}
							else
							{
								$this->session->set_flashdata('error', 'Record could not be added.');
				...
				$data['invoiccedetails']['invoice_no'] 			= (10000+$data['orderdata']['Id']);
				$data['invoiccedetails']['invoice_date'] 		= substr($data['orderdata']['date'],0,10);
				$data['invoiccedetails']['login_id'] 			= $data['authordata']['username'];
				$data['invoiccedetails']['billed_to'] 			= $data['authordata']['name'].
																  $data['authordata']['username'].
																  get_corresponing_value('countries','name',$data['authordata']['country'],'Id')
																  .$data['authordata']['contact_no'];
															
				$data['invoiccedetails']['manuscript_no']		= manuscriptNo($data['orderdata']['mid']);
				$data['invoiccedetails']['manuscript_name']		= get_corresponing_value('manuscript','title',$data['orderdata']['mid'],'Id');
				$data['invoiccedetails']['quantity'] 			= $data['orderdata']['qty'];				
				$data['invoiccedetails']['unitprice'] 			= $data['orderdata']['unitprice'];
				$data['invoiccedetails']['discount'] 			= $data['orderdata']['discount'];				
				$data['invoiccedetails']['total'] 				= ($data['orderdata']['unitprice'] - $data['orderdata']['discount']);
				$data['invoiccedetails']['total_amount'] 		= $data['orderdata']['payment'];				
				$data['invoiccedetails']['grand_total_amount'] 	= ($data['invoiccedetails']['total']*$data['orderdata']['qty']);								
				
				//pr($data,false);																
		}
			
		$this->load->view($this->view_folder.'editinvoice',$data);
