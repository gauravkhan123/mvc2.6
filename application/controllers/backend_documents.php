...
class Backend_Documents extends Secure_Controller {
	...
	function __construct()
	{
		...
		$this->db_id 			= "Id";
		$this->controller 		= $this->config->item('backend').'/documents';
		$this->alias			= "";
		$this->view_folder		= "backend/content/documents/";
	...
	{
		$data['coloums'] = $this->basic_model->list_result(array('Id','title','ms_word_file','publish'),$this->table);
			...
			if($this->input->post('submit')!='')
			{
				...
				///////////////////////
					...
					if ($handle->uploaded) {
						...
						$handle->Process('media/documents/');				
				...
				///////////////////////					
					...
					$this->form_validation->set_rules('ms_word_file', 'ms_word_file', 'trim|required');						
			...
			if($this->input->post('submit')!='')
			{
				...
				///////////////////////
					...
					if ($handle->uploaded) {
						...
						unlink_image($this->table,'media/documents/',$id,$field="ms_word_file");
						$handle->Process('media/documents/');				
						$_POST['ms_word_file'] = $handle->file_dst_name;
	...
	public function view($id)
	{
		...
		{	
			...
			$data['data'] = $this->basic_model->view_record(array('title','ms_word_file','date'),$this->table,$this->db_id,$id);	
