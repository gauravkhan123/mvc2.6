...
class Backend_Pictures extends Secure_Controller {
	...
	function __construct()
	{
		...
		$this->db_id 			= "id";
		$this->controller 		= $this->config->item('backend').'/pictures';
		$this->alias			= "";
		$this->view_folder		= "backend/content/pictures/";
	...
	{
		$data['coloums'] = $this->basic_model->list_result(array('id','title','image','publish'),$this->table);
		...
		{	
			...
			if($this->input->post('submit')!='')
			{
				...
				///////////////////////
					...
					if ($handle->uploaded)
					{
						unlink_image($this->table,'media/pictures/',$id,$field="image");
						$handle->Process('media/pictures/');						
						$_POST['image'] = $handle->file_dst_name;
						$handle->Clean();		
					}
