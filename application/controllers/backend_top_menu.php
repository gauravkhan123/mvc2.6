...
class Backend_Top_Menu extends Secure_Controller {
...

	{
		...
		$this->controller 		= $this->config->item('backend').'/top_menu';

		$this->alias			= "";

		$this->view_folder		= "backend/menus/top_menu/";



		$this->first_field		= "link_name";
...

	{
		...
		$data['coloums'] = $this->basic_model->list_result(array('id','link_name','link_url','link_position','publish'),$this->table);
...

	{
...

			{
					...
					$this->form_validation->set_rules('link_name', 'Title', 'trim|required');	

					$this->form_validation->set_rules('link_url', 'Link Url', 'trim|required');						

					$this->form_validation->set_rules('link_position', 'Title Tag', 'trim');
...

	{
		...
		{	
			...
			$data['data'] = $this->basic_model->view_record(array('link_name','link_url','link_position'),$this->table,$this->db_id,$id);	
